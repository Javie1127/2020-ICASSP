function [fdcomm,radar,radar_comm, PAR] = parameters
...
K                           = 8; % The length of the radar code; or the number of PRIs
N_UL                        = 4*ones(I,1); % Number of the UL UE antennas
N_DL                        = 2*ones(J,1); % Number of the DL UE antennas
%Htr                         = zeros(Mr,Nr);
d_DL                        = N_DL;% number of stream for each DL UE d_DL(i) <= N_DL(i)   
fdcomm.ULstream_num         = 2*ones(I,1);
N                           = 10;% Number of range cells
n                           = 4; % CUT index
%% Power 
sigma_0                     = 1; % noise power
SNR_rtr                     = 10*ones(Mr,Nr); % db radar TX signal to noise ratio at the radar RX
SNR_BS_DL                   = 10*ones(J,1);
SNR_UL_BS                   = 10*ones(I,1);
P_r                         = zeros*ones(Mr,1);
P_DL                        = 0;
P_UL                        = zeros(I,1);
...
for mr = 1 : Mr
    ...
    for nr = 1:Nr
        H_rtr (mr,nr) = sqrt(eta_t/2)*(randn(1)+1i*randn(1));
    end
end
Qr = zeros(Mr,K,Nr);
for nr = 1:Nr
...
PAR.rho = 2; % a number from [1, K], i.e. the maximum permissible PAR
%-----------------------------------------------------------------%
%% FD Comm
% UL channels
...
fdcomm.R_DL = 0;
%% Subgradient method 
fdcomm.lambda_UL = zeros(I,K);
fdcomm.lambda_DL = zeros(K,1);
fdcomm.mu_UL = zeros(I,K);
fdcomm.mu_DL = zeros(J,K);
...
%% Initialization of the precoders 
PAR = ones(Mr,1); %% PAR leve;
[P_UL_ini, P_DL_ini, A_ini] = ini_ap(Mr, H_UL, H_DL, d_DL, d_UL, P_DL,P_UL,P_r,K,PAR,N);
...
for mr = 1 : Mr
    for jj = 1:J
        ...
        H_r_DL{mr,jj} = sqrt(eta_r_DL(mr,jj)^2/(kappa_DL-1)/2)*(randn(N_DL(jj),1)+1i*randn(N_DL(jj),1))+sqrt(kappa_DL/(kappa_DL+1)*mu_DL(mr,jj)*ones(N_DL(jj),1));
    end
end
% radar - BS Rx
%eta_r_BS = ones(Mr,1);
...
for ii = 1:I
    for nr = 1:Nr
        H_UL_r(:,nr) = sqrt(eta_U(ii,nr)/2).*(randn(N_UL(ii),1)+1i*randn(Nc,1));
    end
end
